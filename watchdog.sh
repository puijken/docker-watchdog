...
# Graceful exit handler — stop the container if watchdog is terminated
...
while true; do
  ...
  if [ "$LAST_STATE" != "running" ]; then
    ...
    LAST_STATE="running"
    LAST_ACTIVE=$(date +%s)  # set once when starting monitoring loop
  fi

  # === Inner monitoring loop ===
  while docker inspect -f '{{.State.Running}}' "$CONTAINER" 2>/dev/null | grep -q true; do
    # Execute CHECK_CMD inside the target container
